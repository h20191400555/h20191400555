...
#define C_R_VID 0xaaaa    //card reader MXT USB Device          
#define C_R_PID  0x8816                

#define HP_VID  0x03f0       //hp pendrive
#define HP_PID  0x6d40

#define SANDISK_VID 0x0781	//sandisk pendrive
...
#define RD_CAP_LEN          0x08     //recived byte number after capacity command (Read capacity length)
#define RD_INQ_LEN          0x24     // recived byte number after Inquiry command (Read inquire length)
#define RD_DATA             0X200    //simply to stored sector length (Read data)
...
int only_inq()
{
	...
	cbw->dCBWTag = tag;
	exp_tag=tag;
	cbw->dCBWDataTransferLength = RD_INQ_LEN; //
	...
	//--------Inquiry receive----//
	ej=usb_bulk_msg(udev, usb_rcvbulkpipe(udev,endpoint_in_addr), (unsigned char*)buffer, RD_INQ_LEN, &size, 1000);
...
int only_cap()
{
	...
	cbw->dCBWTag = tag;
	exp_tag=tag;
	cbw->dCBWDataTransferLength = RD_CAP_LEN; //
	cdb_len = cdb_length[cdb[0]];	
	cbw->bCBWCBLength = cdb_len;
	memcpy(cbw->CBWCB, cdb, cdb_len);
   ...
   while(r!=0 && ct<5)  //if not sent first time
	...
	i=usb_bulk_msg(udev, usb_rcvbulkpipe (udev,endpoint_in_addr), (unsigned char*)buffer,  RD_CAP_LEN, &size1, 10000);
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	if(id->idProduct == C_R_PID)
	{
		printk(KERN_INFO "Known USB drivedetected\n");
		printk(KERN_INFO "card reader plugged in\n");
	}
	else if(id->idProduct == HP_PID)
	{
		printk(KERN_INFO "Known USB drivedetected\n");
		printk(KERN_INFO "HP plugged in\n");
	}
	...
	for(i=0;i<interface->cur_altsetting->desc.bNumEndpoints;i++)
	{
		...
		if((epAttr & USB_ENDPOINT_XFERTYPE_MASK)==USB_ENDPOINT_XFER_BULK)
		{
			if(epAddr & USB_DIR_IN){
				...
				printk(KERN_INFO "EndPoint %d is Bulk IN address=%x \n", i,endpoint_in_addr); 
				
		
				}
			else{
				endpoint_out_addr=epAddr;
				printk(KERN_INFO "EndPoint %d is Bulk OUT address=%x\n", i, endpoint_out_addr);
...
{	
	...
	printk(KERN_INFO "--------UAS USB Block storage Driver Inserted------\n");
	return 0;
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Bhargav Kumar Tripathy(2019H1400081H) & Ratna Kumari C(2019H1400555H)");
MODULE_DESCRIPTION("USB block storage Device")
